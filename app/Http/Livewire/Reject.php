...
class Reject extends Component
{
    ...
    {
        $this->output = collect(DB::select("select output_rejects.*, so_det.size, COUNT(output_rejects.id) output from `output_rejects` left join `so_det` on `so_det`.`id` = `output_rejects`.`so_det_id` where `master_plan_id` = '".$this->orderInfo->id."' and `status` = 'NORMAL' group by so_det.id"));
    }
    ...
    public function render(SessionManager $session)
    {
        ...
        // Defect types
        $this->defectTypes = $this->defectTypes = DB::table("output_defect_types")->leftJoin(DB::raw("(select reject_type_id, count(id) total_reject from output_rejects where updated_at between '".date("Y-m-d", strtotime(date("Y-m-d").' -10 days'))." 00:00:00' and '".date("Y-m-d")." 23:59:59' group by reject_type_id) as rejects"), "rejects.reject_type_id", "=", "output_defect_types.id")->whereRaw("(hidden IS NULL OR hidden != 'Y')")->orderBy('total_defect', 'desc')->orderBy('defect_type')->get();

        // Defect areas
        $this->defectAreas = $this->defectAreas = DB::table("output_defect_areas")->leftJoin(DB::raw("(select reject_area_id, count(id) total_reject from output_rejects where updated_at between '".date("Y-m-d", strtotime(date("Y-m-d").' -10 days'))." 00:00:00' and '".date("Y-m-d")." 23:59:59' group by reject_area_id) as rejects"), "rejects.reject_area_id", "=", "output_defect_areas.id")->whereRaw("(hidden IS NULL OR hidden != 'Y')")->orderBy('total_defect', 'desc')->orderBy('defect_area')->get();
